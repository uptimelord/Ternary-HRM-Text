...
Exp56 fixed the soundness hole (sound carry closure after neural elimination),
but the neural threshold ran FIRST and routinely eliminated the true carry path
before closure could enforce it -> closure correctly turned the state into a
conflict, so coverage was zero.

Exp57 flips the order. Per solve step:
1. sound_carry_closure runs FIRST (removes provably-inconsistent candidates).
2. closure_led_eliminate: the neural threshold may remove a candidate ONLY from
   cells that closure left with >=2 candidates, and may NEVER empty a cell or
   remove a unique survivor that closure proved necessary (closure has veto).
3. branch_pin on remaining ambiguous cells.
4. sound_carry_closure again to propagate the branch / re-validate elimination.

The neural net's role shrinks to guiding which cell/value to branch on; the
deterministic closure owns carry truth. Because a sound closure is always the
last narrowing applied before the solved check, returned singletons are
arithmetically valid by construction (wrong_returns = 0).
...
def sound_carry_closure(rows: list[dict[str, Any]], alive: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Close the lattice under sound two-digit addition constraints.

    Only removes candidates. Never adds. If the true carry/digit path was
    already eliminated by the neural step or a branch, this returns conflict.
    """
...
def closure_led_eliminate(alive: torch.Tensor, keep_logits: torch.Tensor, *, threshold: float) -> torch.Tensor:
    """Neural elimination under closure veto.

    The neural threshold may remove a candidate ONLY from cells that closure
    left with >=2 alive candidates. It may never empty a cell and may never
    remove a unique survivor (a cell closure already pinned to one candidate).
    Closure therefore has veto power over every neural elimination.
    """
    valid = cell_candidate_mask(alive.device).unsqueeze(0)
    keep = torch.sigmoid(keep_logits) >= threshold
    proposed = alive & keep & valid

    counts = alive.sum(dim=2, keepdim=True)
    ambiguous = counts >= 2  # only closure-ambiguous cells may be narrowed

    # Outside ambiguous cells (unique survivors), keep closure's result intact.
    result = torch.where(ambiguous, proposed, alive)

    # Never empty a cell: if neural elimination would empty an ambiguous cell,
    # veto the whole cell back to its pre-elimination (closed) candidate set.
    emptied = result.sum(dim=2, keepdim=True) == 0
    result = torch.where(emptied, alive, result)
    return result
...
def rollout_alive(
    ...
    *,
    ...
    for _ in range(max_solve_steps):
        ...
        # 1. sound closure first
        alive, closure_conflict = sound_carry_closure(rows, alive)
        # 2. neural elimination only on closure-ambiguous cells (closure veto)
        alive = closure_led_eliminate(alive, keep_logits, threshold=threshold)
        # 3. branch on remaining ambiguous cells
        if branch:
            alive = branch_pin(alive, keep_logits)
        # 4. close again to propagate the branch / re-validate elimination
        alive, post_conflict = sound_carry_closure(rows, alive)
        closure_conflict = closure_conflict | post_conflict
        counts = alive.sum(dim=2)
        conflicted = closure_conflict | (counts == 0).any(dim=1) | (torch.sigmoid(conflict_logits) >= cls_threshold)
