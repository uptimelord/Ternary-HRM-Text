...
Exp56 made the carry closure sound, but the on-policy loop kept eliminating the
true path before closure could enforce carry truth, so coverage stayed at zero
(every split collapsed to conflict).

Exp58 keeps the Exp56 sound carry-closure and loss byte-identical and only
changes HOW the training lattice is produced:

- Phase A (first --curriculum-steps, default 60% of --steps): train from the TOP
  lattice (full candidate sets). The network is supervised to narrow from the
  complete lattice -- a top-state warmup that teaches confident logits.
- Phase B (remaining steps): train on-policy exactly as Exp56 does
  (rollout-then-supervise).

Hypothesis: the warmup teaches the net confident logits so that, once on-policy
elimination begins, threshold elimination stops killing the true path.
    ...
    *,
    ...
    for _ in range(max_solve_steps):
        ...
        if branch:
            ...
            alive, branch_conflict = sound_carry_closure(rows, alive)
            closure_conflict = closure_conflict | branch_conflict
        counts = alive.sum(dim=2)
...
def train_one_seed(args: argparse.Namespace, seed: int) -> dict[str, Any]:
    ...
    for step in range(1, args.steps + 1):
        ...
        # Phase A: top-state warmup (supervise narrowing from the full lattice).
        # Phase B: on-policy rollout-then-supervise exactly as Exp56.
        in_curriculum_phase = step <= args.curriculum_steps
        if (not in_curriculum_phase) and args.on_policy_steps > 0:
            alive = rollout_alive(
                model,
                batch,
                encoded,
        ...
        if step % args.eval_every == 0 or step == args.steps:
            ...
            if not args.smoke:
                phase = "A_top" if in_curriculum_phase else "B_onpolicy"
                print(f"step={step} phase={phase} loss={last_loss:.4f} valid_argmax_acc={valid['acc']:.4f}")
...
def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.curriculum_steps is None:
        args.curriculum_steps = int(round(0.6 * args.steps))
    args.curriculum_steps = max(0, min(args.curriculum_steps, args.steps))
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Phase A top-state warmup steps; defaults to 60%% of --steps.",
    )
