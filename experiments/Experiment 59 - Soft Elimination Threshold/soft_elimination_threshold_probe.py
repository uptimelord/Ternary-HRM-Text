...
Exp56 closed the soundness hole with the sound carry closure, but the neural
elimination threshold (default 0.1) was the primary eliminator. On weak/untrained
logits that meant the true carry/digit path was killed before closure ran, so
closure correctly turned almost everything into conflict (coverage zero).

Exp59 keeps the carry closure byte-identical and instead makes neural elimination
*soft*:
- lower the neural threshold default from 0.1 to 0.02 so weak/untrained logits do
  NOT prematurely kill candidates; let sound closure be the primary eliminator.
- add --neural-elim {on,off}. When off, threshold_eliminate removes NOTHING (only
  re-masks to the candidate domain), so sound closure + branch-pin are the SOLE
  eliminators (pure-deduction baseline).
...
def threshold_eliminate(
    ...
    *,
    threshold: float,
    neural_elim: bool = True,
) -> torch.Tensor:
    if not neural_elim:
        # Pure-deduction baseline: remove nothing, only re-mask to the candidate
        # domain. Sound closure + branch-pin become the sole eliminators.
        return alive & cell_candidate_mask(alive.device).unsqueeze(0)
...
def rollout_alive(
    ...
    *,
    ...
    for _ in range(max_solve_steps):
        ...
        alive = threshold_eliminate(alive, keep_logits, threshold=threshold, neural_elim=neural_elim)
...
def train_one_seed(args: argparse.Namespace, seed: int) -> dict[str, Any]:
    ...
    neural_elim = args.neural_elim == "on"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--neural-elim", choices=["on", "off"], default="on")
